...
{
    long_t val = 0;
    if (op == A_ADD) val = argB + argA;
    if (op == A_SUB) val = argB - argA;
    if (op == A_AND) val = argB & argA;
    if (op == A_XOR) val = argB ^ argA;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    long_t valE, valA, valB, valM;
    if (!get_byte_val(sim->m, sim->pc, &codefun)) 
        return STAT_ADR;
    cond_t cond = GET_FUN(codefun);
    switch (icode) {
      case I_HALT: /* 0:0 */
	return STAT_HLT;
	break;
